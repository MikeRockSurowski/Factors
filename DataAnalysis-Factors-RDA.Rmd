---
title: "DataAnalysis-Factors-RDA"
...
alkaliEarths <- c('Sr', 'Ba', 'Rb')
UTHaf <- c('U', 'Th', 'Hf')
rareEarthsYttr <- c("La", "Ce", "Pr", "Nd", "Sm", "Eu", "Gd", "Tb", 
                    "Dy", "Ho", "Y", "Er", "Tm", "Yb", "Lu")

rare.earths.light <- c("La", "Ce", "Pr", "Nd", "Sm")
rare.earths.mid <- c("Eu", "Gd", "Tb", "Dy", "Ho")
rare.earths.heavy <- c("Er", "Tm", "Yb", "Lu", "Y")

metalOxides <- c('Fe', 'Mn', 'V', 'Cu', 'Zn', 'Ga')
...
# paasData <- read.csv('Data/paas3mm.csv', sep=';')
paasData <- read.csv('Data/paas_sing_profiles_3mm.csv', sep=';')
reeAnomalies <- read.csv('Data/reeAnomalies_sing_profiles_3mm.csv', sep=';')
# yData <- read.csv('Data/spot_fullinfo.csv', sep=';')
...
ratios <- subset(reeAnomalies, select=-c(SampleID, Line, Spot, SpotID))
...
# Subcompositions
Datasets with subcompositions of interest
...
ppm.alkaliEarths <- data.ppm[, alkaliEarths]
ppm.rareEarthsYttr <- data.ppm[, rareEarthsYttr]

ppm.rare.earths.light <- data.ppm[, rare.earths.light]
ppm.rare.earths.mid <- data.ppm[, rare.earths.mid]
ppm.rare.earths.heavy <- data.ppm[, rare.earths.heavy]

ppm.UTHaf <- data.ppm[, UTHaf]
ppm.metalOxides <- data.ppm[, metalOxides]

```

## PAAS subcompositions

```{r}
paas.alkaliEarths <- data.PAAS[, alkaliEarths]
paas.rareEarthsYttr <- data.PAAS[, rareEarthsYttr]
# paas.rareEarthsYGa <- data.PAAS[, rareEarthsYGa]

paas.rare.earths.light <- data.PAAS[, rare.earths.light]
paas.rare.earths.mid <- data.PAAS[, rare.earths.mid]
paas.rare.earths.heavy <- data.PAAS[, rare.earths.heavy]

paas.UTHaf <- data.PAAS[, UTHaf]
paas.metalOxides <- data.PAAS[, metalOxides]
```

## Centering and scaling

```{r}
# dataPAAS[is.na(dataPAAS)] <- 0.000001

# ratios transformation

shiftRatios <- abs(sapply(ratios, min)) + 1
dataShiftRatios <- sweep(ratios, 2, shiftRatios, FUN='+')
logData.Ratios <- log(dataShiftRatios)
logData.Scaled.Ratios <- scale(logData.Ratios, center=TRUE, scale=TRUE)
...
ggsave('Plots/RDA biplot - all.variables.png', 
...
ggsave('Plots/RDA biplot - all.variables ZOOM.png', 
...
## Locality, conditional thickness and skeletal part
...
ggsave('Plots/RDA biplot - full - only locality and conditions.png',
...
ggsave('Plots/RDA biplot - full - only locality and conditions ZOOM.png',
...
ggsave('Plots/RDA biplot - cond. locality.png',
...
ggsave('Plots/RDA biplot - cond. locality ZOOM.png',
...
ggsave('Plots/RDA PAAS ratios biplot.png',
...
ggsave('Plots/RDA PAAS ratios biplot ZOOM.png',
...
# Results
Here we demonstrated that besides local diagenetic conditions, the skeletal parts of particular type do differ highly in their geochemistry. Besides, we also found that the thickness of the cortex is another factor, that influences the chemical composition of the bone independently - which is especially true for short profiles.